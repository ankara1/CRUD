...
@Controller
@RequestMapping("/secundario")																//habra veces que no se podra modificar la ruta del @RequestMapping del metodo aun habiendo otro igual en otro controlador, lo cual lanzara una excepcion al ejecutar.. la solucion es establecer una ruta relativa agregando otro @RequestMapping pero arriba de la clase principal   
public class OtroControlador2 {
	...
	@RequestMapping("/procesar2")															//url a pasar al form action o href (procesar2 se repite en ambos controladores lo que lanzara una excepcion amenos que se establezcan rutas relativas
...
/*	public String procesar2(HttpServletRequest request, Model modelo) {						//paso por parametros un HttpServletRequest para poder usar su metodo para capturar un parametro del formulario y paso un modelo para 
		...
		nombre+=" es el peor alumno";														//al nombre capturado nomas le concatene otro string
		String mensaje = "quien es el peor alumno? " + nombre;								//cree otro string con una pregunta previa y le agregue el nombre capturado con su concatenacion
